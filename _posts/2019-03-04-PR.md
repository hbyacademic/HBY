...
[dataCount,atrb]=size(C);%number of features except for class name
cv = cvpartition(name,'HoldOut',ratio);
...
idx=test(cv,1); %the split result (1/0 represent training/testing data)
