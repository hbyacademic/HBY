...
- First, use `subplot` to display multiple plots in one figure. 
- Second, choose each two features to form a 2D space, and instances in the first feature are treated as the point x, and those in the second feature are treated as y. All the points (x,y) span the 2D space.
