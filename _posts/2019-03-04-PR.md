...
w=[0 0 0 0]; %initial feature weight vector

%to find the value of the weight vector
%all the training data are correctly classified
while(correct~=length(dataTrainName))
    for k=1:length(dataTrainName)
        %do adjustment when meeting a misclassified data
        if(w(1)+dataTrain(k,1)*w(2)+dataTrain(k,2)*w(3)+dataTrain(k,3)*w(4)>0)
            if(strcmp(dataTrainName(k),'Iris-setosa'))
                w=w-[1,dataTrain(k,1),dataTrain(k,2),dataTrain(k,3)];
                correct=0;
                break;
            else
                correct=correct+1;
