...
2. select `an appropriate map` and `camera exposure time`
 The exposure time setting changes if the estimated lighting
 condition is too dark or too bright
 if it is too dark, then increasing exposure time
 if it is too bright, then decreasing exposure time
